...
const transformProductToDraft = (formData: TProductForm): TProductForm => {
  return {
    ...formData,
    product_name: `${formData.product_name}（複製）`,
    product_status: activeStatusEnums.enum.inactive,
  };
};
...
const ProductDraftForm = () => {
  ...
  const { id: sourceProductId } = useParams({ strict: false });
  ...
  const { data: sourceProductData, isLoading: isLoadingSourceProduct } =
    useGetProduct(sourceProductId || "");
  ...
  const sourceProductName = useMemo(() => {
    if (
      !sourceProductData ||
      !Array.isArray(sourceProductData) ||
      sourceProductData.length === 0
    ) {
      return "";
    }
    return sourceProductData[0].product_name || "";
  }, [sourceProductData]);
  ...
  const data = useMemo((): TProductForm => {
    if (
      ...
      !Array.isArray(sourceProductData) ||
      sourceProductData.length === 0 ||
      isLoadingSourceProduct
    ) {
      return DEFAULT_VALUES;
    }

    const sourceProduct = sourceProductData[0];
    const formData = transformProductToFormData(sourceProduct);
    return transformProductToDraft(formData);
  }, [sourceProductData, isLoadingSourceProduct]);
  ...
  useEffect(() => {
    if (
      ...
      const sourceProduct = sourceProductData[0];
      const formData = transformProductToFormData(sourceProduct);
      const draftData = transformProductToDraft(formData);

      form.reset(draftData);

      form.setValue("product_type", draftData.product_type, {
        shouldValidate: true,
        shouldDirty: false,
      });
      form.setValue("product_status", draftData.product_status, {
        shouldValidate: true,
        shouldDirty: false,
      });
      form.setValue("vendor_id", draftData.vendor_id || [], {
        shouldValidate: true,
        shouldDirty: false,
      });
      form.setValue("tag_id", draftData.tag_id || [], {
        shouldValidate: true,
        shouldDirty: false,
      });
    }
  }, [form, sourceProductData, isLoadingSourceProduct]);
        ...
        <Paper className="flex w-full flex-col gap-4">
          <div className="flex flex-col gap-2">
            <h3 className="text-3xl font-bold">
              {t("pages.product.productDraft.title", {
                productName: sourceProductName,
              })}
            </h3>
            <p className="text-sm text-muted-foreground">
              {t("pages.product.productDraft.description")}
            </p>
          </div>
          <TextInput
            className="flex-1"
